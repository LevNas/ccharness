...
1. Dry run from the repository root and show the plan to the user:
   ...
   Use `--lang en` for English templates (`tone.md` is Japanese-only), `--target DIR` when not at the repository root, `--no-settings` to leave `.claude/settings.json` alone.
2. Existing rule files are never overwritten. A "differs" diff is information for the user, not something to resolve automatically.
